...
class DesktopWorkbench:
    ...
    def _receipt(self, action: str, ok: bool, details: dict[str, Any]) -> dict[str, Any]:
        receipt = {
            "action": action,
            "ok": bool(ok),
            "details": sanitize_activity_details(details),
        }
        self._mutate(
            "desktop_receipt",
    ...
    def open_project(self) -> dict[str, Any]:
        ...
        self._mutate(
            ...
            lambda data: data.__setitem__("project", project),
            project,
        )
    ...
    def perform_inference(self, request: dict[str, Any]) -> Any:
        """Perform provider inference only; no canonical state or Tk access occurs here."""
        if self.inference_fabric is None:
            raise RuntimeError("inference fabric is not configured")
    ...
    def fail_message(self, run_id: str, error: BaseException | str) -> None:
        ...
        self._mutate(
            "desktop_run_failed",
            mutate_run,
            {"run_id": run_id, "error": public_error},
        )
        if was_running:
            self.emit_activity(
                "run_failed",
    ...
    def _set_control(self, state: str) -> dict[str, Any]:
        stopped_run: dict[str, Any] = {}
        finished_at = self._utc_now()

        def mutate_control(current: dict[str, Any]) -> None:
            current["control_state"] = state
        ...
        if stopped_run:
            self.emit_activity(
                "run_stopped",
                "Active agent run stopped by user",
                status="stopped",
                run_id=str(stopped_run["run_id"]),
                phase="stopped",
...
def run_acceptance_e2e(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="alinacoder-acceptance-") as td:
        ...
        workbench.verify_goal_criterion(
            goal.goal_id,
            "c1",
            {"path": "acceptance.txt", "present_in_diff": "acceptance.txt" in diff},
        )
        workbench.verify_goal_criterion(
            goal.goal_id,
            "c2",
        ...
        committed = workbench.commit_main(
            "acceptance: AlinaCoder packaged end-to-end mission"
        )
        workbench.pause()
        paused_snapshot = workbench.snapshot()
        workbench.close()

        restarted = DesktopWorkbench(
            workspace,
            state_path=state_path,
            session_id="packaged-e2e",
        )
