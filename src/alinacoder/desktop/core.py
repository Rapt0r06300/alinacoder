...
class AutomationRegistry:
    ...
    def default_ids()->dict[str,str]:
        names=['composer','send','voice','goal','pause','resume','stop','takeover','project','sessions','activity','plan','context','diff','tests','git','receipts','run_inspector','provider_status','resource_status','diagnostics'];return {n:f'alinacoder.{n}' for n in names}
...
class WorkbenchModel:
    _ACTIONS=frozenset({'open_project','send_message','start_goal','pause','resume','stop','takeover','view_activity','view_plan','edit_plan','view_context','view_diff','run_tests','commit_main','view_git','view_receipts','view_run_inspector','configure_provider','configure_local_model','open_diagnostics','select_artifact','voice_input'})
...
def self_test()->dict:
    ids=AutomationRegistry.default_ids();actions=WorkbenchModel().available_actions();required={'send_message','start_goal','stop','commit_main','view_activity'};return {'ok':len(ids.values())==len(set(ids.values())) and required.issubset(actions),'automation_ids':ids,'actions':sorted(actions)}
